function sol = ResultofThreeVarTwoPoly()    
%This function implements reference paper section 4.1
...
display('getting final result of 3 variables and 2 polynomials');
v1data =  dlmread('.\TestData\1-testdata-3v2p.txt');
v1data =  v1data(1:6536,:);
v1class = v1data(:,1);  %real labels
v2data =  dlmread('.\TestData\2-testdata-3v2p.txt');
v2data =  v2data(1:6536,:);
v2class = v2data(:,1);   %real labels
v3data =  dlmread('.\TestData\3-testdata-3v2p.txt');
v3data =  v3data(1:6536,:);
v3class = v3data(:,1);   %real labels
v4data =  dlmread('.\TestData\4-testdata-3v2p.txt');
v4data =  v4data(1:6536,:);
v4class = v4data(:,1);   %real labels
v5data =  dlmread('.\TestData\5-testdata-3v2p.txt');
v5data =  v5data(1:6536,:);
v5class = v5data(:,1);   %real labels
v6data =  dlmread('.\TestData\6-testdata-3v2p.txt');
v6data =  v6data(1:6536,:);
v6class = v6data(:,1);    %real labels

pre1data =  dlmread('.\TestResult\1-test-tag-3v2p.txt');
pre1data =  pre1data(1:end,:);
pre1class = pre1data(:,1);    %predict labels
trust1class = pre1data(:,2:3);    %predict labels's credibility
pre2data =  dlmread('.\TestResult\2-test-tag-3v2p.txt');
pre2data =  pre2data(1:end,:);
pre2class = pre2data(:,1);
trust2class = pre2data(:,2:3);
pre3data =  dlmread('.\TestResult\3-test-tag-3v2p.txt');
pre3data =  pre3data(1:end,:);
pre3class = pre3data(:,1);
trust3class = pre3data(:,2:3);
pre4data =  dlmread('.\TestResult\4-test-tag-3v2p.txt');
pre4data =  pre4data(1:end,:);
pre4class = pre4data(:,1);
trust4class = pre4data(:,2:3);
pre5data =  dlmread('.\TestResult\5-test-tag-3v2p.txt');
pre5data =  pre5data(1:end,:);
pre5class = pre5data(:,1);
trust5class = pre5data(:,2:3);
pre6data =  dlmread('.\TestResult\6-test-tag-3v2p.txt');
  ...
  %Calculate the same number of predicted values as the original label
     ...
     if(j==2)
      ...
      if(j==6)
          ...
          dlmwrite('.\FinalResult\PredictLabel-3v2p.txt',a,'-append','delimiter',' ','precision','%d');     
end

rate=0;
rate=n/6536;
dlmwrite('.\FinalResult\PredictAccuracy-3v2p.txt',rate,'delimiter',' ','precision','%d');
display('Result of 3v2p Accuracy:');
display(rate);
