...
display('getting final result of Algebraic Surface Gallery');
v1data =  dlmread('.\TestData\1-testdata-ASG.txt');
v1data =  v1data(1:814,:);
v1class = v1data(:,1);  %real labels
v2data =  dlmread('.\TestData\2-testdata-ASG.txt');
v2data =  v2data(1:814,:);
v2class = v2data(:,1);
v3data =  dlmread('.\TestData\3-testdata-ASG.txt');
v3data =  v3data(1:814,:);
v3class = v3data(:,1);
v4data =  dlmread('.\TestData\4-testdata-ASG.txt');
v4data =  v4data(1:814,:);
v4class = v4data(:,1);
v5data =  dlmread('.\TestData\5-testdata-ASG.txt');
v5data =  v5data(1:814,:);
v5class = v5data(:,1);
v6data =  dlmread('.\TestData\6-testdata-ASG.txt');
v6data =  v6data(1:814,:);
v6class = v6data(:,1);

pre1data =  dlmread('.\TestResult\1-test-tag-ASG.txt');
pre1data =  pre1data(1:end,:);
pre1class = pre1data(:,1);       %predict labels
trust1class = pre1data(:,2:3);     %predict labels's credibility
pre2data =  dlmread('.\TestResult\2-test-tag-ASG.txt');
pre2data =  pre2data(1:end,:);
pre2class = pre2data(:,1);
trust2class = pre2data(:,2:3);
pre3data =  dlmread('.\TestResult\3-test-tag-ASG.txt');
pre3data =  pre3data(1:end,:);
pre3class = pre3data(:,1);
trust3class = pre3data(:,2:3);
pre4data =  dlmread('.\TestResult\4-test-tag-ASG.txt');
pre4data =  pre4data(1:end,:);
pre4class = pre4data(:,1);
trust4class = pre4data(:,2:3);
pre5data =  dlmread('.\TestResult\5-test-tag-ASG.txt');
pre5data =  pre5data(1:end,:);
pre5class = pre5data(:,1);
trust5class = pre5data(:,2:3);
pre6data =  dlmread('.\TestResult\6-test-tag-ASG.txt');
  ...
  %Calculate the same number of predicted values as the original label
     ...
     if(j==2)
      ...
      if(j==6)
          ...
          dlmwrite('.\FinalResult\PredictLabel-ASG.txt',a,'-append','delimiter',' ','precision','%d');     
end

rate=0;
rate=n/814;
dlmwrite('.\FinalResult\PredictAccuracy-ASG.txt',rate,'delimiter',' ','precision','%d');
display('Result of ASG Accuracy:');
display(rate);
