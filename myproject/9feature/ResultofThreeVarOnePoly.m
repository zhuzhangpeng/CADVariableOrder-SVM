function sol = ResultofThreeVarOnePoly()    
...
display('getting final result of 3 variables and 1polynomial');
v1data =  dlmread('.\TestData\1-testdata-3v1p.txt');
v1data =  v1data(26145:32680,:);
v1class = v1data(:,1);      %real labels
v2data =  dlmread('.\TestData\2-testdata-3v1p.txt');
v2data =  v2data(26145:32680,:);
v2class = v2data(:,1);
v3data =  dlmread('.\TestData\3-testdata-3v1p.txt');
v3data =  v3data(26145:32680,:);
v3class = v3data(:,1);
v4data =  dlmread('.\TestData\4-testdata-3v1p.txt');
v4data =  v4data(26145:32680,:);
v4class = v4data(:,1);
v5data =  dlmread('.\TestData\5-testdata-3v1p.txt');
v5data =  v5data(26145:32680,:);
v5class = v5data(:,1);
v6data =  dlmread('.\TestData\6-testdata-3v1p.txt');
v6data =  v6data(26145:32680,:);
v6class = v6data(:,1);

pre1data =  dlmread('.\TestResult\1-test-tag-3v1p.txt');
pre1data =  pre1data(1:end,:);
pre1class = pre1data(:,1);    %predict labels
trust1class = pre1data(:,2:3);      %predict labels's credibility
pre2data =  dlmread('.\TestResult\2-test-tag-3v1p.txt');
pre2data =  pre2data(1:end,:);
pre2class = pre2data(:,1);
trust2class = pre2data(:,2:3);
pre3data =  dlmread('.\TestResult\3-test-tag-3v1p.txt');
pre3data =  pre3data(1:end,:);
pre3class = pre3data(:,1);
trust3class = pre3data(:,2:3);
pre4data =  dlmread('.\TestResult\4-test-tag-3v1p.txt');
pre4data =  pre4data(1:end,:);
pre4class = pre4data(:,1);
trust4class = pre4data(:,2:3);
pre5data =  dlmread('.\TestResult\5-test-tag-3v1p.txt');
pre5data =  pre5data(1:end,:);
pre5class = pre5data(:,1);
trust5class = pre5data(:,2:3);
pre6data =  dlmread('.\TestResult\6-test-tag-3v1p.txt');
  ...
  %Calculate the same number of predicted values as the original label
     ...
     if(j==2)
      ...
      if(j==6)
          ...
          dlmwrite('.\FinalResult\PredictLabel-3v1p.txt',a,'-append','delimiter',' ','precision','%d');     
end

rate=n/6536;
dlmwrite('.\FinalResult\PredictAccuracy-3v1p.txt',rate,'delimiter',' ','precision','%d');
display('Result of 3v1p Accuracy:');
display(rate);
