function  fun = traindata()   %get training data and test data
   ...
   data=dlmread('.\TrainingData\9feature-3v2p.txt');
   resdata=data(:,12:end);
   ...
   display('gettingtest data of 3 variables and 1 polynomial');
     data2=dlmread('.\TrainingData\9feature-3v1p.txt');
   resdata2=data2(:,12:end);
   ...
   for i=1:1:6
       ...
       Feature=data2(:,3:11);   %Regularized featurevector
       vectordata = (Feature - repmat(min(Feature,[],1),size(Feature,1),1))*spdiags(1./(max(Feature,[],1)-min(Feature,[],1))',0,size(Feature,2),size(Feature,2));
   ...
   display('getting  test data of Algebraic Surface Gallery');
     data3=dlmread('.\TrainingData\9feature-ASG.txt');
   resdata3=data3(:,12:end);
   ...
   for i=1:1:6
       ...
       Feature=data3(:,3:11);   %Regularized featurevector
     %vectordata = (Feature - repmat(min(Feature,[],1),size(Feature,1),1))*spdiags(1./(max(Feature,[],1)-min(Feature,[],1))',0,size(Feature,2),size(Feature,2));
       vectordata = Feature(:)';
   ...
   data4=dlmread('.\TrainingData\9feature-2v2p.txt');
   resdata4=data4(:,12:end);
   ...
   for i=1:1:6
       ...
       Feature=data4(:,3:11);   %Regularized featurevector
       %vectordata = (Feature - repmat(min(Feature,[],1),size(Feature,1),1))*spdiags(1./(max(Feature,[],1)-min(Feature,[],1))',0,size(Feature,2),size(Feature,2));
